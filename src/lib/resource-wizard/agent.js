                ...
                properties: {
                    changes: {
                        ...
                        items: {
                            type: generative_ai_1.SchemaType.OBJECT,
                            properties: {
                                action: { type: generative_ai_1.SchemaType.STRING },
                                user_id: { type: generative_ai_1.SchemaType.STRING },
                                project_id: { type: generative_ai_1.SchemaType.STRING },
                                hours: { type: generative_ai_1.SchemaType.NUMBER },
                                week_start: { type: generative_ai_1.SchemaType.STRING }
                            }
                        },
 ...
 */
async function processCommand(userInput, context) {
    ...
    try {
        ...
        if (functionCalls && functionCalls.length > 0) {
            ...
            const actions = functionCalls.map(fc => ({
                tool: fc.name,
                params: fc.args,
                description: generateActionDescription(fc.name, fc.args, context)
            }));
 ...
 */
function generateActionDescription(toolName, params, context) {
    const findUser = (id) => context.users.find(u => u.id === id)?.name || id;
    const findProject = (id) => context.projects.find(p => p.id === id)?.name || id;
    switch (toolName) {
        case 'add_allocation':
            return `Add ${params.hours}h for ${findUser(params.user_id)} on ${findProject(params.project_id)} (week of ${params.week_start})`;
        case 'remove_allocation':
            return `Remove ${findUser(params.user_id)} from ${findProject(params.project_id)} (week of ${params.week_start})`;
        case 'move_allocation':
            const from = params.from_user_id ? `from ${findUser(params.from_user_id)}` : '';
            return `Move ${params.hours}h ${from} to ${findUser(params.to_user_id)} on ${findProject(params.project_id)}`;
        ...
        case 'get_project_status':
            return `Get status for ${findProject(params.project_id)}`;
        default:
 ...
 */
async function continueConversation(userInput, conversationHistory, context) {
