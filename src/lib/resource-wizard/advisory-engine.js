...
exports.evaluateAdvisory = evaluateAdvisory;
function evaluateAdvisory(request, context) {
    const factors = [];
...
function assessRoleMatch(user, project) {
    // Simple heuristic - could be much smarter
    const specialty = user.specialty_notes?.toLowerCase() || '';
    const projectName = project.name.toLowerCase();
    // Very basic matching
    if (specialty.includes('design') && projectName.includes('design'))
        return true;
    if (specialty.includes('develop') && (projectName.includes('dev') || projectName.includes('build')))
        return true;
    return true; // Default to good match if we can't determine
}
function findAlternatives(request, context) {
