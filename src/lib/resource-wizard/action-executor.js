...
function debug(stage, message, data) {
    if (DEBUG) {
 ...
 */
async function executeActions(actions, executorUserId, orgId) {
 ...
 */
async function executeAction(action, executorUserId, orgId) {
    switch (action.tool) {
        case 'add_allocation':
            return executeAddAllocation(action.params, executorUserId, orgId);
        case 'remove_allocation':
            return executeRemoveAllocation(action.params, orgId);
        case 'move_allocation':
            return executeMoveAllocation(action.params, executorUserId, orgId);
        case 'bulk_update_allocations':
            return executeBulkUpdate(action.params, executorUserId, orgId);
        // Query operations - execute and return data
        case 'get_user_availability':
            return executeGetUserAvailability(action.params, orgId);
        case 'get_user_allocations':
            return executeGetUserAllocations(action.params, orgId);
        case 'get_project_status':
            return executeGetProjectStatus(action.params);
        case 'suggest_coverage':
            return executeSuggestCoverage(action.params, orgId);
        case 'search_users':
 ...
 */
async function executeAddAllocation(params, executorUserId, orgId) {
 ...
 */
async function executeRemoveAllocation(params, orgId) {
 ...
 */
async function executeMoveAllocation(params, executorUserId, orgId) {
 ...
 */
async function executeBulkUpdate(params, executorUserId, orgId) {
    ...
    for (let i = 0; i < changes.length; i++) {
        ...
        switch (change.action) {
            ...
            case 'update':
                ...
                break;
            default:
                debug('bulk', `❌ Unknown action: ${change.action}`);
                result = {
 ...
 */
async function updateAllocationHours(change, executorUserId, orgId) {
    ...
    if (!existing) {
        ...
        return executeAddAllocation({ user_id, project_id, hours, week_start }, executorUserId, orgId);
    }
 ...
 * Log an audit entry
 */
async function logAudit(orgId, entityType, entityId, action, userId, changes) {
 ...
 */
async function executeGetUserAvailability(params, orgId) {
 ...
 */
async function executeGetUserAllocations(params, orgId) {
 ...
 */
async function executeGetProjectStatus(params) {
    ...
    return {
        ...
        data: {
            ...
            phases: include_phases ? (project.phases || []).map(p => ({
                id: p.id,
                name: p.name,
 ...
 */
async function executeSuggestCoverage(params, orgId) {
    ...
    const absentUserJobTitle = absentAllocations[0].user?.job_title;
    const absentUserName = absentAllocations[0].user?.name;
 ...
 */
async function previewActions(actions, _orgId) {
    ...
    for (const action of actions) {
        ...
        if (params.user_id)
            userIds.add(params.user_id);
        if (params.from_user_id)
            userIds.add(params.from_user_id);
        if (params.to_user_id)
            userIds.add(params.to_user_id);
        if (params.project_id)
            projectIds.add(params.project_id);
    }
    ...
    for (const alloc of currentAllocations || []) {
        const userName = alloc.user?.name || alloc.user_id;
        if (!before[userName])
            before[userName] = [];
        before[userName].push({
            project: alloc.project?.name || alloc.project_id,
            hours: alloc.planned_hours
        });
    }
