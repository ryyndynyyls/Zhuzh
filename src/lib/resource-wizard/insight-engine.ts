...
import type { ResourceWizardContext, ResourceInsight, WizardUser, WizardProject } from './types';
...
function analyzeOverallocations(context: ResourceWizardContext): ResourceInsight[] {
  ...
  for (const user of context.users) {
    ...
    const overallocatedWeeks: Array<{ week: string; hours: number; overage: number }> = [];
