...
exports.buildResourceWizardContext = buildResourceWizardContext;
 ...
 */
function getWeekStarts(startDate, weeks) {
    const starts = [];
 ...
 */
async function buildResourceWizardContext(options) {
 ...
 */
async function fetchUsersWithAllocations(orgId, startWeek, endWeek, focusUserIds) {
    // Build user query
    ...
    return users.map(user => {
        const userAllocations = (allocations || [])
            ...
            phase_name: a.phase?.name || undefined
        }));
 ...
 */
async function fetchProjects(orgId, focusProjectId) {
    ...
    const hoursUsedByProject = new Map();
 ...
 */
async function searchUsers(orgId, query, role) {
 ...
 */
async function searchProjects(orgId, query, activeOnly = true) {
    const searchQuery = query.toLowerCase().trim();
    let projectQuery = supabase
        .from('projects')
 ...
 */
async function getUserAvailability(orgId, startWeek, endWeek, userId, roleFilter) {
    // Build user query
    let userQuery = supabase
        .from('users')
        .select('id, name, role')
 ...
 */
async function getProjectStatus(projectId, includePhases = false) {
