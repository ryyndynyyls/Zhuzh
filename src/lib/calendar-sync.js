...
exports.syncUserCalendar = syncUserCalendar;
exports.syncOrgCalendars = syncOrgCalendars;
exports.detectFridayOffAttendees = detectFridayOffAttendees;
exports.classifyEvent = classifyEvent;
exports.getWhosOut = getWhosOut;
exports.getWhosOutWeek = getWhosOutWeek;
exports.isUserOut = isUserOut;
exports.getUserWorkingDays = getUserWorkingDays;
const supabase_js_1 = require("@supabase/supabase-js");
const google_calendar_1 = require("./google-calendar");
const supabase = (0, supabase_js_1.createClient)(process.env.VITE_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
// =============================================================================
 ...
 * - Partial matches
 */
async function matchNameToUser(name, orgId, userCache, creatorEmail // Use event creator to disambiguate
) {
    ...
    const firstNameMatches = users.filter(u => u.name.toLowerCase().split(' ')[0] === firstName);
    ...
    if (firstNameMatches.length > 1 && creatorEmail) {
        const creatorMatch = firstNameMatches.find(u => u.email?.toLowerCase() === creatorEmail.toLowerCase());
        if (creatorMatch) {
    ...
    const partialMatch = users.find(u => u.name.toLowerCase().startsWith(nameLower));
    if (partialMatch) {
 ...
 */
async function syncUserCalendar(userId, startDate, endDate) {
    const result = {
        userId,
        userName: 'Unknown',
        eventsProcessed: 0,
    ...
    try {
        ...
        const { tokens, wasRefreshed } = await (0, google_calendar_1.getValidTokens)(user.google_access_token, user.google_refresh_token, new Date(user.google_token_expiry).getTime());
        if (wasRefreshed) {
            await supabase
                .from('users')
                .update({
        ...
        const calendars = await (0, google_calendar_1.listCalendars)(tokens);
        ...
        for (const calendarId of calendarIds) {
            try {
                const events = await (0, google_calendar_1.getCalendarEvents)(tokens, calendarId, startDate, endDate);
        ...
        const eventsToStore = [];
        const seenEventKeys = new Set(); // Dedupe by user_id + calendar_event_id
        for (const event of allEvents) {
            ...
            if (eventType === 'friday_off' && event.attendees) {
                ...
                const matchedUsers = (matchedByEmail || []).map(u => ({ id: u.id, name: u.name }));
 ...
 */
async function syncOrgCalendars(orgId, startDate, endDate) {
 ...
 */
async function detectFridayOffAttendees(orgId, friday) {
    ...
    try {
        const { tokens } = await (0, google_calendar_1.getValidTokens)(user.google_access_token, user.google_refresh_token, new Date(user.google_token_expiry).getTime());
        const dayStart = new Date(friday);
        dayStart.setHours(0, 0, 0, 0);
        const dayEnd = new Date(friday);
        dayEnd.setHours(23, 59, 59, 999);
        const events = await (0, google_calendar_1.getCalendarEvents)(tokens, user.google_calendar_id || 'primary', dayStart, dayEnd);
        const fridayOffEvent = events.find(e => e.summary.toLowerCase().includes('fridays off') ||
        ...
        for (const email of attendeeEmails) {
            ...
            if (firstName && !matchedIds.has(firstName)) {
                const { data: nameMatch } = await supabase
                    ...
                    .eq('org_id', orgId)
                    .eq('is_active', true);
                const matched = nameMatch?.find(u => u.name.toLowerCase().split(' ')[0] === firstName ||
 ...
 */
async function getWhosOut(orgId, date) {
 ...
 */
async function getWhosOutWeek(orgId, weekStart) {
    const result = [];
    for (let i = 0; i < 7; i++) {
 ...
 */
async function isUserOut(userId, date) {
    const dateStr = date.toISOString().split('T')[0];
 ...
 */
async function getUserWorkingDays(userId, weekStart) {
    const workingDays = [];
    const ptoSummary = new Map();
...
// Helper Functions
// =============================================================================
async function getOrgCalendarConfig(orgId) {
    const { data } = await supabase
    ...
    if (data?.pto_detection && data?.recurring_schedules) {
        return {
            pto_detection: data.pto_detection,
            recurring_schedules: data.recurring_schedules
        };
    }
